...
## 12.11.0
- **FEAT**(crop-rotate): Add perspective tilt/skew to the crop-rotate editor (#525). A new tilt tool (`CropRotateTool.tilt`, configurable via `TiltConfigs` / `TiltStyle` / `TiltWidgets`) lets users correct perspective with separate rotate, horizontal and vertical rulers. While tilting, the crop selection is kept fully inside the perspective-warped image by auto-zooming - the minimal zoom that still covers the crop is found per frame - and panning; straightening the image zooms back out to (but never below) the user's manual zoom, and the tilt is clamped when `maxScale` can no longer cover the crop, so the selection never reveals empty area outside the image. The live preview, the exported render (`TransformedContentGenerator`), the auto-zoom bounds math and the darkened overlay all share one transform composition, so the result matches the preview under perspective. Tilt is hidden in the video editor by default (a free-angle perspective transform isn't natively exportable). The angles round-trip through `TransformConfigs` and are exposed on `CompleteParameters` (`tiltRotate` / `tiltHorizontal` / `tiltVertical`) for consumers that re-apply the transforms instead of using the rendered bytes. Additive, non-breaking change.
