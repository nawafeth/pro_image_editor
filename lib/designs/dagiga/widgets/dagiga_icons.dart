  ...
  static Widget eyedropper({
    ...
    return CustomPaint(
      ...
      painter: _StrokePathPainter(
        ...
        paths: const [
          _kEyedropperOutline,
          'M4.46912 1.49414L12.4059 9.43088',
        ],
      ),
    );
  }
...
Path _parseSvgPath(String data) {
  final path = Path();
  final tokens = RegExp(r'[MmLlHhVvCcZz]|-?\d*\.?\d+(?:e[-+]?\d+)?')
      .allMatches(data)
      .map((m) => m.group(0)!)
      .toList();
...
bool _isNumber(String token) =>
    token.isNotEmpty && (token.codeUnitAt(0) == 45 /* - */ ||
